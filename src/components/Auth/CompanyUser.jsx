...
const CompanyUser = () => {
    ...
    const fetchUsers = async (page = 0, size = 10, keyword = "") => {
        try {
            ...
            const response = await Authservices.searchWorkers(params);
    ...
    const handleEditClick = (user) => {
        setEditingUser(user);
        setFormData({
            fullName: user.fullName || `${user.name} ${user.surname}`, // Handle composite name if needed
            email: user.email,
            phone: user.phone,
            active: user.active,
            ...
            .then(() => {
                const updatedUsers = users.map(user =>
                    user.id === editingUser.id
                        ? { ...user, ...formData, name: formData.fullName.split(' ')[0], surname: formData.fullName.split(' ').slice(1).join(' ') } // Optimistic update approximation
                        : user
                );
                setUsers(updatedUsers);
                handleCloseModal();
                fetchUsers(currentPage, 10, searchName); // Refresh to be sure
            })
            .catch(err => {
            ...
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 bg-white dark:bg-gray-800 p-6 rounded-2xl shadow-sm border border-gray-100 dark:border-gray-700 transition-all duration-300 hover:shadow-md">
                <div className="flex items-center gap-4">
                    ...
                    <div>
                        ...
                        <p className="text-sm text-gray-500 dark:text-gray-400">
                            Manage your company staff and their permissions
                        </p>
                    </div>
                </div>
                <div className="flex gap-3">
                    ...
                    <button
                        ...
                        className="inline-flex items-center px-4 py-2 bg-gradient-to-r from-blue-600 to-indigo-600 hover:from-blue-700 hover:to-indigo-700 text-white text-sm font-medium rounded-xl shadow-lg shadow-blue-500/30 transition-all duration-300 transform hover:-translate-y-0.5"
                    >
                ...
                <div className="flex flex-col md:flex-row gap-4 items-center justify-between">
                    <form onSubmit={handleSearchSubmit} className="flex-1 w-full md:max-w-md relative group">
                        ...
                        <input
                            ...
                            className="w-full pl-10 pr-4 py-2.5 bg-gray-50 dark:bg-gray-900 border border-gray-200 dark:border-gray-700 rounded-xl focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 dark:text-white transition-all"
                        />
                    </form>
